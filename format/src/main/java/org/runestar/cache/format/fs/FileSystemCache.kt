...
private constructor(
    ...
    override fun getIndexReference(index: Int): CompletableFuture<IndexReference> {
        ...
        val indexRef = IndexReference(Volume(dataBuffer.get(index, indexEntry)))
    ...
    override fun getVolume(index: Int, archive: Int): CompletableFuture<Volume?> {
        ...
        if (archive >= idxBuffer.entryCount) return CompletableFuture.completedFuture(null)
        val idxEntry = idxBuffer.get(archive) ?: return CompletableFuture.completedFuture(null)
        return CompletableFuture.completedFuture(Volume(dataBuffer.get(archive, idxEntry)))
    }
