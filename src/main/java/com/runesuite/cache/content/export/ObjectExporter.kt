...
class ObjectExporter(cache: ReadableCache, dir: Path) : CacheExporter(cache, dir) {
    ...
    override fun export() {
        cache.getArchive(Index.CONFIGS.id, Index.CONFIGS.OBJECT)?.files?.forEachIndexed { i, f ->
            val obj = ObjectDefinition()
            obj.id = i
            obj.read(f)
            mapper.writeValue(dir.resolve("$i.json").toFile(), obj)
        }
    }
}